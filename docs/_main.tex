...
\addcontentsline{toc}{section}{Electronic Copies of the Workbook}
...
\addcontentsline{toc}{section}{Citing the Workbook}
...
\caption{Example of organizational exercise. Image is of blue group folders (text in {[}{]} brackets), pink single folders (text in ``\,'' quotes), and yellow file types (regular text) sticky notes, each with its own label, organized hierarchically on a wooden desktop.}
\end{figure}
...
\section{Make Spreadsheets More Accessible and Reusable}\label{accessible-spreadsheets}}
...
\section{Write Alt Text for a Scientific Data Visualization}\label{alt-text}}

\textbf{\emph{Description:}} \emph{Writing alt text (short for ``alternative text'') is one of the most basic ways to make a scientific figure accessible to a broader audience by providing a textual description of an image. Alt text is useful when sharing figures on your lab website, on social media, and even when publishing journal articles. Alt text is a basic requirement when sharing digital images.}

\textbf{\emph{Instructions:}} \emph{Answer questions 1-3 for each panel of a figure then assemble the answers using the formula in step 4. This formula for creating alt text comes from Amy Cesal at Nightingale \citep{cesal_writing_2021}. It's less accessible than a more complete description of a visualization or sharing the data underlying a visualization, but it's a quick way to write usable alt text for a chart.}
...
\begin{figure}
\centering
\includegraphics{images/06_AltText.jpg}
\caption{Figure used as the example for this exercise. Figure is from \citep{briney_measuring_2024}}
\end{figure}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{1. In 2-3 words, list the chart type:}

\emph{Example: Column chart}
...
\textbf{2. Briefly summarize the primary data (e.g.~y-axis data) that makes up this chart:}

\emph{Example: Research data availability}
...
\textbf{3. In one sentence, write the main takeaway of the visualization:}

\emph{Example: Research data on the internet disappears at a rate of 2.6\% per year.}
...
\textbf{4. Write out the full alt text as: {[}Answer 1{]} of {[}answer 2{]} where {[}answer 3{]}.}

\emph{Example: Column chart of research data availability where research data on the internet disappears at a rate of 2.6\% per year.}
...
\textbf{5. Where applicable, repeat steps 1-4 for each panel of a multi-panel figure. Note which part of the figure goes with each sentence.}

\emph{Example: Not applicable}
...
\textbf{6. Where possible, include a link to the data underlying the figure.}

\emph{Example: For underlying data, see ``Figure2\_UnavailableByYear.csv'' file at \url{https://doi.org/10.22002/h5e81-spf62}}
...
\textbf{7. Share the alt text anytime you share the figure.}
